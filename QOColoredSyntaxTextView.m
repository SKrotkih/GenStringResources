/* =============================================================================
 FILE:		QOColoredSyntaxTextView.m
 PROJECT:	CocoaTads
...
@interface QOColoredSyntaxTextView (Private)
// Private:
...
-(void)	colorOneLineComment: (NSString*) startCh 
                   inString: (NSMutableAttributedString*) s
                  withColor: (NSColor*) col 
                    andMode: (NSString*)attr;
-(void)	colorCommentsFrom: (NSString*) startCh 
                       to: (NSString*) endCh 
                 inString: (NSMutableAttributedString*) s
                withColor: (NSColor*) col 
                  andMode:(NSString*)attr;
-(void)	colorIdentifier: (NSString*) ident 
               inString: (NSMutableAttributedString*) s
              withColor: (NSColor*) col 
                andMode: (NSString*)attr 
                charset: (NSCharacterSet*)cset;
...
-(void)	colorTagFrom: (NSString*) startCh to: (NSString*)endCh 
            inString: (NSMutableAttributedString*) s
           withColor: (NSColor*) col 
             andMode: (NSString*)attr 
...
@implementation QOColoredSyntaxTextView
...
- (void)didLoadNib
{
	...
	if( [self respondsToSelector: @selector(setUsesFindPanel:)] )
...
{
    return [[self string] dataUsingEncoding: NSMacOSRomanStringEncoding allowLossyConversion:YES];
}
                        ...
                        ofType: (NSString*) aType
{
	...
	sourceCode = [[NSString alloc] initWithData: data 
                                       encoding: NSMacOSRomanStringEncoding]; // Load the new text.
	...
	if(evt)
	{
		...
		if(param)		// This is always false when xCode calls us???
		{
			...
			if(range.lineNum >= 0)
				...
				[self goToRangeFrom: range.startRange 
                             toChar: range.endRange];
		}
	}
...
-(BOOL) textView:(NSTextView *)tv shouldChangeTextInRange:(NSRange)afcr replacementString:(NSString *)rps
{
	if( maintainIndentation )
	{
		...
		[self performSelector: @selector(didChangeText) 
                   withObject: nil 
                   afterDelay: 0.0];	// Queue this up on the event loop. If we change the text here, we only confuse the undo stack.
	}
...
-(IBAction) indentSelection: (id)sender
{
	[[self undoManager] registerUndoWithTarget: self 
                                      selector: @selector(unindentSelection:) 
                                        object: nil];
...
-(IBAction) unindentSelection: (id)sender
{
	...
	[[self undoManager] registerUndoWithTarget: self 
                                      selector: @selector(indentSelection:) 
                                        object: nil];
...
-(void)		recolorRange: (NSRange)range
{
    ...
    [vString addAttributes: vStyles 
                     range: NSMakeRange( 0, [vString length] )];
    [[self textStorage] replaceCharactersInRange: range 
                            withAttributedString: vString];
...
-(void)		oldRecolorRange: (NSRange)range
{
	...
	if( range.length == 0 || recolorTimer )						// And don't like recoloring partially if a full recolorization is pending.
		return;
    ...
    if( vIdents )
    {
        ...
        while( vCurrIdent = [vItty nextObject] )
            [self colorIdentifier: vCurrIdent 
                         inString: vString 
                        withColor: vIdentifierColor
                          andMode: TD_IDENTIFIER_ATTR 
                          charset: vIdentCharset];
    }
    ...
    if( vIdents )
    {
        ...
        while( vCurrIdent = [vItty nextObject] )
            [self colorIdentifier: vCurrIdent 
                         inString: vString 
                        withColor: vIdentifier2Color
                          andMode: TD_IDENTIFIER2_ATTR 
                          charset: vIdentCharset];
    }
    
    // Colorize comments, strings etc, obliterating any identifiers inside them:
    [self colorStringsFrom: @"\"" 
                        to: @"\"" 
                  inString: vString 
                 withColor: vStringColor 
                   andMode: TD_DOUBLE_QUOTED_STRING_ATTR 
             andEscapeChar: vStringEscapeCharacter];   // Strings.
	
    // Colorize colorize any tags:
    if( vTagStart )
        [self colorTagFrom: vTagStart 
                        to: vTagEnd 
                  inString: vString 
                 withColor: vTagColor 
                   andMode: TD_TAG_ATTR 
              exceptIfMode: vTagIgnoredStyle];
    ...
    if( vIdents )
    {
        ...
        NSEnumerator* vItty = [vIdents objectEnumerator];
        while( vCurrIdent = [vItty nextObject] )
            [self colorOneLineComment: vCurrIdent 
                             inString: vString 
                            withColor: vPreprocessorColor 
                              andMode: TD_PREPROCESSOR_ATTR];	// TODO Preprocessor directives should make sure they're at the start of a line, and that whitespace follows the directive.
    }
    
    // Comments:
    if( vOneLineCommentStart )
        [self colorOneLineComment: vOneLineCommentStart 
                         inString: vString 
                        withColor: vCommentColor 
                          andMode: TD_ONE_LINE_COMMENT_ATTR];
    if( vBlockCommentStart )
        [self colorCommentsFrom: vBlockCommentStart 
                             to: vBlockCommentEnd 
                       inString: vString 
                      withColor:vCommentColor 
                        andMode: TD_MULTI_LINE_COMMENT_ATTR];
    if( vBlockComment2Start )
        [self colorCommentsFrom: vBlockComment2Start 
                             to: vBlockComment2End 
                       inString: vString 
                      withColor:vComment2Color 
                        andMode: TD_MULTI_LINE_COMMENT2_ATTR];
           ...
           andEscapeChar: (NSString*)vStringEscapeCharacter
{
    ...
    while( ![vScanner isAtEnd] )
    {
        ...
        while( !vIsEndChar && ![vScanner isAtEnd] )	// Loop until we find end-of-string marker or our text to color is finished:
        {
            ...
            if( ![vScanner scanString: endCh 
                           intoString: nil] )	// But skip this char before that.
                NS_VOIDRETURN;
                  ...
                  andMode: (NSString*)attr
{
	NS_DURING
    NSScanner* vScanner = [NSScanner scannerWithString: [s string]];
    NSDictionary* vStyles = [NSDictionary dictionaryWithObjectsAndKeys: col, NSForegroundColorAttributeName,
                                           attr, TD_SYNTAX_COLORING_MODE_ATTR,
                                           nil];
                ...
                charset: (NSCharacterSet*)cset
{
    ...
    while( ![vScanner isAtEnd] )
    {
        ...
        [vScanner scanUpToString: ident 
                      intoString: nil];
        vStartOffs = [vScanner scanLocation];
        if( ![vScanner scanString:ident 
                       intoString:nil] )
            NS_VOIDRETURN;
...
-(void)	colorTagFrom: (NSString*) startCh 
                  to: (NSString*)endCh 
            inString: (NSMutableAttributedString*) s
           withColor: (NSColor*) col 
             andMode: (NSString*)attr 
...
{
	return [NSDictionary dictionaryWithObject: [NSFont userFixedPitchFontOfSize: 12.0] 
...
{
    NSRange theRange = [[self string] rangeOfString: aString];
    if (theRange.location != NSNotFound)
    {
        [self scrollRangeToVisible: theRange];
        [self setSelectedRange: theRange];
        [self showFindIndicatorForRange: theRange];
    }
}
